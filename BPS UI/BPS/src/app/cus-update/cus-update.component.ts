...
export class CusUpdateComponent implements OnInit {
  ...
  billDetails:any;
  deletedBill:any;
  constructor(private customerUpdateService:CustomerUpdateService,private regService:CustomerRegService,private route:Router, private billService:GenerateBillService) { }
  ngOnInit() {
  ...
  deleteCustomer(){
    this.customerUpdateService.deleteCustomer(this.customerId).subscribe(data => {
      this.delete=data;
      if(this.delete['customerId']===null){
        this.billService.getAmountToPay(this.customerId).subscribe(data =>{
          this.billDetails=data;
          this.billId=this.billDetails[0]['billId'];
          this.billService.deleteBill(this.billId).subscribe(data =>{
            this.deletedBill=data;
          })
        })
        alert("Customer "+this.customerId+" deleted");
  ...
  updateCustomer(){
   ...
   //method to cancel the process and redirect to home
   cancel(){
